...
from trees.decision_tree import (
    DecisionTreeClassifier,
    DecisionTreeRegressor, encode_forest_helper, decode_forest_helper,
    _RegressionNode, _ClassificationNode, _compute_split, _compute_split_regressor, construct_subtree, _NodeInfo,
...
class BaseRandomForest(BaseEstimator):
    ...
    def fit(self, x, y):
        ...
        self.try_features = _resolve_try_features(self.try_features, x.shape[1])
        ...
        for n_estimator in range(self.n_estimators):
            branches[n_estimator] = [[x, y, self.base_node()]]
            self.nodes_info[str(n_estimator)] = []
            selection_indexes.append(_sample_selection(x, random_state=self.random_state, bootstrap=self.bootstrap))
        num_buckets = x._n_blocks[0] * x._n_blocks[1]
        for i in range(self.distr_depth):
            for n_estimator in range(self.n_estimators):
                branches_pair = []
                for idx, branch_data in enumerate(branches[n_estimator]):
                    ...
                    if self.n_classes is not None:
                        ...
                        actual_node.content = len(self.nodes_info[str(n_estimator)])
                        splits_computed = []
                        ...
                        node_info, results = _compute_split_regressor(x, y, range_min=self.range_min,
                                                                      ...
                                                                      number_attributes=self.number_attributes,
                                                                      indexes_selected=selection_indexes[n_estimator],
                                                                      number_split_points=int(
                                                                          self.n_split_points * (i + 1)),
                                                                      split_computation=self.split_computation,
                                                                      random_state=self.random_state)
                        actual_node.content = len(self.nodes_info[str(n_estimator)])
                        splits_computed = []
                        ...
                        for k, sides in enumerate(results):
                            ...
                            splits_computed = []
                        self.nodes_info[str(n_estimator)].append(node_info)
                branches[n_estimator] = branches_pair
        for n_estimator in range(self.n_estimators):
            for branch in branches[n_estimator]:
    ...
    def predict(self, x):
        ...
        if self.hard_vote:
            for x_row in x._iterator(axis=0):
                tree_predictions = []
                for n_estimator in self.nodes_info.keys():
                    tree_predictions.append(_predict_tree_class(x_row._blocks, self.nodes_info[n_estimator], 0, self.n_classes))
            ...
            for x_row in x._iterator(axis=0):
                tree_predictions = []
                for n_estimator in self.nodes_info.keys():
                    tree_predictions.append(_predict_proba_tree(x_row._blocks, self.nodes_info[n_estimator], 0, self.n_classes))
            ...
            for x_row, y_row in _paired_partition(x, y):
                ...
                for n_estimator in self.nodes_info.keys():
                    tree_predictions.append(
                        _predict_tree_class(x_row._blocks, self.nodes_info[n_estimator], 0, self.n_classes))
...
class RandomForestRegressor(BaseRandomForest):
    ...
    def score(self, x, y, collect=False):
        ...
        for x_row, y_row in _paired_partition(x, y):
            ...
            for n_estimator in range(self.n_estimators):
                tree_predictions.append(
                    _predict_tree_class(x_row._blocks, self.nodes_info[str(n_estimator)], 0, self.n_classes))
...
@constraint(computing_units="${ComputingUnits}")
@task(y_blocks={Type: COLLECTION_IN, Depth: 2}, predictions=COLLECTION_IN, returns=1)
