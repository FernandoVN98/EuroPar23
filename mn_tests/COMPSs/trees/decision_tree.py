...
from trees.terasort import terasort
...
class BaseDecisionTree:
    ...
    def fit(self, x, y):
        ...
        nodes_info = []
        selection = _sample_selection(x, random_state=self.random_state, bootstrap=self.bootstrap)
        ...
        for i in range(self.distr_depth):
            ...
            for idx, branch_data in enumerate(branches):
                ...
                node_info, results = _compute_split(x, y, n_classes=self.n_classes, range_min=self.range_min,
                                                    range_max=self.range_max, total_length=self.total_length,
                                                    num_buckets=int(num_buckets/(i+1)), m_try=self.try_features,
                                                    number_attributes=self.number_attributes,
                                                    indexes_selected=selection, number_split_points=int(self.n_split_points*(i+1)),
        ...
        for branch in branches:
            ...
            construct_subtree(x, y, actual_node, self.try_features, self.distr_depth, max_depth=self.max_depth,
                              random_state=self.random_state)
    ...
    def predict(self, x, collect=False):
        ...
        for x_block in x._blocks:
            block_predictions.append(_predict_tree_class(x_block, self.nodes_info, 0, self.n_classes))
        if collect:
    ...
    def predict_proba(self, x, collect=False):
        """Predicts class probabilities for a row block using a fitted tree.
                ...
                predicted_proba : list
                    A list with the predicted probabilities for the given samples.
                    It contains a numpy array (if collect=True)
                    or Future object (if collect=False) for each of the blocks
                    in the ds-array to predict. Thus the length of the list is the same
                    as the number of blocks the ds-array contains.
                    The shape inside each prediction is (len(x.reg_shape[0]), self.n_classes).
        ...
        block_predictions = []
        for x_block in x._blocks:
            block_predictions.append(_predict_proba_tree(x_block, self.nodes_info, 0, self.n_classes))
        if collect:
            block_predictions = compss_wait_on(block_predictions)
        return block_predictions
...
class DecisionTreeRegressor(BaseDecisionTree):
    ...
    def fit(self, x, y):
        ...
        for i in range(self.distr_depth):
            ...
            for idx, branch_data in enumerate(branches):
                x, y, actual_node = branch_data
                node_info, results = _compute_split_regressor(x, y, range_min=self.range_min, range_max=self.range_max,
                                                              total_length=self.total_length, num_buckets=int(num_buckets/(i+1)),
...
def _compute_split_regressor(x, y, num_buckets=4, range_min=0, range_max=1, total_length=None, indexes_selected=None,
                             number_attributes=2, m_try=2,
                             number_split_points=100, split_computation="raw", random_state=1,):
    indexes_to_try = []
    random_state = check_random_state(random_state)
    untried_indices = np.setdiff1d(np.arange(number_attributes), indexes_to_try)
    ...
    if num_buckets < 1:
        num_buckets = 1
    results = terasort(x, indexes_to_try, range_min=range_min, range_max=range_max, indexes_selected=indexes_selected,
                       num_buckets=num_buckets)
    split_points_per_attribute = []
    for i in range(len(results[0])):
        split_points_per_attribute.append(get_split_point_various_attributes_bucket(results[:, i], total_length=None, number_split_points=number_split_points, split_computation=split_computation))
    ...
    for idx, split_values in enumerate(split_points_per_attribute):
        ...
        if isinstance(x, Array):
            for index_blocks, block_s in enumerate(zip(x._blocks, y._blocks)):
                idx_selected = indexes_selected[indexes_selected < (index_blocks + 1) * x._reg_shape[0]]
                ...
                classes_per_split(block_x, block_y, split_values, left_class, right_class, indexes_to_try,
                                                              idx_selected[
                                                                  idx_selected >= (index_blocks) * x._reg_shape[
                                                                      0]] % x._reg_shape[0], regression=True)
            ...
            for block_x, block_y in zip(x, y):
                left_class = [object() for _ in range(len(indexes_to_try))]
                right_class = [object() for _ in range(len(indexes_to_try))]
                classes_per_split(block_x, block_y, split_values, left_class, right_class, indexes_to_try, regression=True)
                partial_results_left[idx].append(left_class)
                partial_results_right[idx].append(right_class)
    ...
    for idx in range(partial_results_right_array.shape[0]):
        for j in range(partial_results_right_array.shape[2]):
            global_gini_values, produces_split = merge_partial_results_compute_mse_both_sides(partial_results_left_array[idx, :, j],
    ...
    [compss_delete_object(b) for results in partial_results_right for result in results for b in result]
    [compss_delete_object(b) for results in partial_results_left for result in results for b in result]
    best_attribute, position_m_g, bucket_minimum_gini, minimum_mse = get_minimum_measure(store_mse_values,
                                                                                         m_try,
                                                                                         gini=False)
    optimal_split_point = select_optimal_split_point(best_attribute, position_m_g, split_points_per_attribute,
                                                     bucket_minimum_gini)
    ...
    if isinstance(x, Array):
        for block_x, block_y in zip(x._blocks, y._blocks):
            right_x, right_y, left_x, left_y, compress_r, len_compress_r, compress_l, len_compress_l = apply_split_points_to_blocks_regression(block_x, block_y, best_attribute,
                                                                            optimal_split_point, indexes_to_try)
        ...
        for block_x, block_y in zip(x, y):
            right_x, right_y, left_x, left_y, compress_r, len_compress_r, compress_l, len_compress_l = apply_split_points_to_blocks_regression(block_x, block_y,
                                                                            best_attribute,
                                                                            optimal_split_point, indexes_to_try)
            rights_x.append([right_x])
            rights_y.append([right_y])
    ...
    generate_nodes_with_data_compressed_regression(node_info, found_solution, right_sums, right_lengths, left_sums,
                                                   left_lengths, minimum_mse,
                                                   best_attribute, indexes_to_try, optimal_split_point)

    final_rights_x[0] = rights_x
    final_rights_y[0] = rights_y
    final_lefts_x[0] = lefts_x
    final_lefts_y[0] = lefts_y
    evaluate_exception(found_solution, node_info, final_rights_x, final_rights_y, final_lefts_x, final_lefts_y)
    ...
    return node_info, [[final_lefts_x[0], final_lefts_y[0]], [final_rights_x[0], final_rights_y[0]]]


def _compute_split(x, y, n_classes=None, num_buckets=4, range_min=0, range_max=1, total_length=None,
    ...
    for i in range(len(
            results[0])):
        split_points_per_attribute.append(get_split_point_various_attributes_bucket(results[:, i], total_length=None,
    ...
    for idx in range(partial_results_right_array.shape[0]):
        for j in range(partial_results_right_array.shape[2]):
            global_gini_values, produces_split = merge_partial_results_compute_gini_both_sides(partial_results_left_array[idx, :, j],
                                                                                                                         partial_results_right_array[idx, :, j],
                                                                                                                         n_classes)
    ...
    [compss_delete_object(b) for results in partial_results_right for result in results for b in result]
    [compss_delete_object(b) for results in partial_results_left for result in results for b in result]
    best_attribute, position_m_g, bucket_minimum_gini, minimum_ginis = get_minimum_measure(store_gini_values,
                                                                                         m_try,
                                                                                         gini=True)
    optimal_split_point = select_optimal_split_point(best_attribute, position_m_g, split_points_per_attribute,
                                                     bucket_minimum_gini)
    ...
    if isinstance(x, Array):
        for block_x, block_y in zip(x._blocks, y._blocks):
            right_x, right_y, left_x, left_y, compressed_r_y, compressed_l_y = apply_split_points_to_blocks(block_x, block_y, best_attribute,
                                                                            optimal_split_point, indexes_to_try, n_classes)
            rights_x.append([right_x])
            rights_y.append([right_y])
        ...
        for block_x, block_y in zip(x, y):
            right_x, right_y, left_x, left_y, compressed_r_y, compressed_l_y = apply_split_points_to_blocks(block_x, block_y,
                                                                            best_attribute,
                                                                            optimal_split_point, indexes_to_try, n_classes)
            rights_x.append([right_x])
            rights_y.append([right_y])
        ...
        [compss_delete_object(x_data[0]) for x_data in x]
        [compss_delete_object(y_data[0]) for y_data in y]
    generate_nodes_with_data_compressed(node_info, found_solution, compressed_right, compressed_left,
                                                n_classes, minimum_ginis,
                                                best_attribute, indexes_to_try, optimal_split_point)
    final_rights_x[0] = rights_x
    final_rights_y[0] = rights_y
    final_lefts_x[0] = lefts_x
...
def _predict_tree_class(x, node, node_content_num, n_classes=None, rights=0, depth=0):
    ...
    if isinstance(node_content, _NodeInfo):
        if isinstance(node_content.get(), _LeafInfo):
            ...
            if n_classes is not None:
                ...
                l_msk = x[:, node_content.get().index:(node_content.get().index + 1)] <= node_content.get().value
                pred[l_msk.flatten(), :] = _predict_tree_class(x[l_msk.flatten(), :], node, node_content_num,
                                                               n_classes=n_classes,
...
@task(x=COLLECTION_IN, node=COLLECTION_IN, returns=1)
def _predict_proba_tree(x, node, node_content_num, n_classes=None, rights=0, depth=0):
    if node_content_num == 0:
        node_content_num = node_content_num + 1
    else:
        node_content_num = node_content_num * 2 + rights
    ...
    if isinstance(node_content, _NodeInfo):
        if isinstance(node_content.get(), _LeafInfo):
            single_pred = node_content.get().frequencies / node_content.get().size
            return np.tile(single_pred, (len(x), 1))
        elif isinstance(node_content.get(), _InnerNodeInfo):
            pred = np.empty((x.shape[0], n_classes), dtype=np.float64)
            l_msk = x[:, node_content.get().index:(node_content.get().index + 1)] <= node_content.get().value
            pred[l_msk.flatten(), :] = _predict_proba_tree(x[l_msk.flatten(), :], node, node_content_num,
...
@task(found_solution=INOUT, node_info=INOUT, rights_x=COLLECTION_INOUT, rights_y=COLLECTION_INOUT,
      lefts_x=COLLECTION_INOUT, lefts_y=COLLECTION_INOUT, returns=1)
def evaluate_exception(found_solution, node_info, rights_x, rights_y, lefts_x, lefts_y):
...
def generate_nodes_with_data_compressed_regression(node_info, found_solution, y_r, y_r_occ, y_l, y_l_occ, gini_value,
    ...
    if (np.sum(y_l_occ) + np.sum(y_r_occ)) <= 4:
        ...
        found_solution[0] = 1
    elif np.sum(y_r_occ) == 0:
        node_info.set(_compute_leaf_info((np.sum(y_l) + np.sum(y_r)) / (np.sum(y_l_occ) + np.sum(y_r_occ)), None,
                                          occurrences=np.sum(y_l_occ) + np.sum(y_r_occ)))
        found_solution[0] = 1
    elif np.sum(y_l_occ) == 0:
        node_info.set(_compute_leaf_info((np.sum(y_l) + np.sum(y_r)) / (np.sum(y_l_occ) + np.sum(y_r_occ)), None,
                                          occurrences=np.sum(y_l_occ) + np.sum(y_r_occ)))
        found_solution[0] = 1
    elif attribute_to_split is None:
        node_info.set(_compute_leaf_info((np.sum(y_l) + np.sum(y_r)) / (np.sum(y_l_occ) + np.sum(y_r_occ)), None,
                                          occurrences=np.sum(y_l_occ) + np.sum(y_r_occ)))
        found_solution[0] = 1
    else:
        node_info.set(_InnerNodeInfo(indexes_to_try[attribute_to_split], optimal_split_point))
        found_solution[0] = 0


@constraint(computing_units="${ComputingUnits}")
@on_failure(management='IGNORE')
@task(node_info=INOUT, found_solution=INOUT, y_r=COLLECTION_IN, y_l=COLLECTION_IN, indexes_to_try=COLLECTION_IN)
def generate_nodes_with_data_compressed(node_info, found_solution, y_r, y_l, n_classes, gini_value, attribute_to_split,
    ...
    if (np.sum(data_compressed_left) + np.sum(data_compressed_right)) <= 4:
        node_info.set(_compute_leaf_info(data_compressed_left + data_compressed_right, n_classes))
        found_solution[0] = 1
    elif np.sum(data_compressed_right) == 0:
        node_info.set(_compute_leaf_info(data_compressed_left + data_compressed_right, n_classes))
        found_solution[0] = 1
    elif np.sum(data_compressed_left) == 0:
        node_info.set(_compute_leaf_info(data_compressed_left + data_compressed_right, n_classes))
        found_solution[0] = 1
    elif attribute_to_split is None:
        node_info.set(_compute_leaf_info(data_compressed_left + data_compressed_right, n_classes))
...
def apply_split_points_to_blocks_regression(x_block, y_block, best_attribute, optimal_value, indexes_to_try):
    if optimal_value is None:
        ...
        return None, None, np.block(x_block), np.block(y_block), np.array([0]), np.array([0]), compress_l, len_compress_l
    if x_block is None:
        return None, None, None, None, np.array([np.nan]), np.array([np.nan]), np.array([np.nan]), np.array([np.nan])
    else:
        x_block = np.block(x_block)
        y_block = np.block(y_block)
    left_x = x_block[x_block[:, indexes_to_try[best_attribute]] < optimal_value]
    ...
    del x_block
    del y_block
    return right_x, right_y, left_x, left_y, compress_r, len_compress_r, compress_l, len_compress_l
...
def apply_split_points_to_blocks(x_block, y_block, best_attribute, optimal_value, indexes_to_try, n_classes):
    ...
    if optimal_value is None:
        ...
        return None, None, np.block(x_block), np.block(y_block), aggregate_r, aggregate
    if x_block is None:
...
def select_optimal_split_point(best_attribute, position_m_g, split_points, bucket_minimum_gini):
...
def merge_partial_results_compute_mse_both_sides(partial_results_l, partial_results_r):
    ...
    number_occurrences = [occurrences[1] for occurrences in concatted_values_l]
    mse_values = []
    for individual_values, value in zip(value_to_compute_mse, concatted_values_l):
        mse_values.append(np.sum(np.square(np.subtract(individual_values, value[0]/value[1]))))
        del value
    ...
    for k in range(len(partial_results_r[0])):
        ...
        value_to_compute_mse.append(value_to_mse)
    number_occurrences_r = [occurrences[1] for occurrences in concatted_values_r]
    mse_values_r = []
    for individual_values, value in zip(value_to_compute_mse, concatted_values_r):
        mse_values_r.append(np.sum(np.square(np.subtract(individual_values, value[0]/value[1]))))
        del value
    del concatted_values_r
    if mse_values is None:
        return np.array([np.inf]), False
    return np.add(mse_values, mse_values_r), np.array(
        [number_occurrences_r[i] != 0 and number_occurrences[i] != 0 for i in range(len(mse_values))])

@constraint(computing_units="${ComputingUnits}")
@task(partial_results_l={Type: COLLECTION_IN}, partial_results_r={Type: COLLECTION_IN}, returns=2)
def merge_partial_results_compute_gini_both_sides(partial_results_l, partial_results_r, n_classes):
    if partial_results_l[0] is None or len(partial_results_l[0]) < 1:
    ...
    for k in range(len(partial_results_l[0])):
        ...
        for j in range(len(partial_results_l)):
            if len(partial_results_l[j][k]) > 0:
                value_to_concat[:len(partial_results_l[j][k])] = value_to_concat[:len(partial_results_l[j][k])] + partial_results_l[j][k]
        concatted_values_l.append(value_to_concat)
    number_occurrences = [np.sum(occurrences).astype(int) for occurrences in concatted_values_l]
    gini_values = []
    for value in concatted_values_l:
        gini_values.append(gini_function_compressed(value, np.arange(n_classes)))
    if partial_results_r[0] is None or len(partial_results_r[0]) < 1:
        return np.array([5]), False
    if partial_results_r[0][0] is None:
        return np.array([5]), False
    ...
    for k in range(len(partial_results_r[0])):
        value_to_concat = np.zeros(n_classes)
        for j in range(len(partial_results_r)):
            value_to_concat[:len(partial_results_r[j][k])] = value_to_concat[:len(partial_results_r[j][k])] + partial_results_r[j][k]
        concatted_values_r.append(value_to_concat)
    gini_values_r = []
    for value in concatted_values_r:
        gini_values_r.append(gini_function_compressed(value, np.arange(n_classes)))
    number_occurrences_r = [np.sum(occurrences).astype(int) for occurrences in concatted_values_r]
    del concatted_values_r
    return np.array([(number_occurrences_r[i] / (number_occurrences_r[i] + number_occurrences[i]) * gini_values_r[i]) + (
            number_occurrences[i] / (number_occurrences_r[i] + number_occurrences[i]) * gini_values[i])
                     if number_occurrences[i] >= 4 and number_occurrences_r[i] >= 4 else 5 for i in range(len(gini_values))]), \
           np.array([number_occurrences_r[i] != 0 and number_occurrences[i] != 0 for i in range(len(gini_values))])


@constraint(computing_units="${ComputingUnits}")
@task(x_block=COLLECTION_IN, y_block=COLLECTION_IN, split_points=COLLECTION_IN, number_classes_l=COLLECTION_INOUT, number_classes_r=COLLECTION_INOUT)
def classes_per_split(x_block, y_block, split_points, number_classes_l, number_classes_r, indexes_to_compare, indexes_to_select=None, regression=False):
    if x_block is None or len(x_block) == 0 or np.all(split_points == None):
    ...
    if regression:
        for idx, attribute_split_points in enumerate(split_points):
            ...
            for value in attribute_split_points:
                attribute_splittings_l.append(np.array([np.mean(y_block[x_block[:, idx] < value, 0]),
                                                        np.sum(y_block[x_block[:, idx] < value, 0]),
                                                        len(y_block[x_block[:, idx] < value, 0])]))
                attribute_splittings_r.append(np.array([np.mean(y_block[x_block[:, idx] >= value, 0]),
                                                        np.sum(y_block[x_block[:, idx] >= value, 0]),
                                                        len(y_block[x_block[:, idx] >= value, 0])]))
        ...
        for idx, attribute_split_points in enumerate(split_points):
            ...
            attribute_splittings_r = []
            for value in attribute_split_points:
                attribute_splittings_l.append(np.bincount(y_block[x_block[:, idx] < value, 0].astype(int)))
                attribute_splittings_r.append(np.bincount(y_block[x_block[:, idx] >= value, 0].astype(int)))
            number_classes_l[idx] = attribute_splittings_l
            number_classes_r[idx] = attribute_splittings_r
    del x_block
    del y_block
...
def get_split_point_various_attributes_bucket(unique_values, total_length, number_split_points=100,#TODO EVALUAR TOTAL LENGTH, TAL VEZ SOBRA
    ...
    for idx, bucket in enumerate(unique_values):
        ...
        if total_length is not None:
            number_split_points_actual = int(number_split_points * len(bucket) / total_length)
        else:
        ...
        if split_computation == "raw":
            ...
            distance_between_split_points = int(len(sample_blocks) / number_split_points_actual)
            if distance_between_split_points == 0:
                sample_blocks_list.append(sample_blocks)
            else:
                sample_blocks_list.append(sample_blocks[0::distance_between_split_points])
...
def construct_subtree(x, y, actual_node, m_try, depth, max_depth=25, random_state=0):
    if max_depth == np.inf:
...
def encode_forest_helper(obj):
    if isinstance(obj, (DecisionTreeClassifier, DecisionTreeRegressor, _Node,
                        _ClassificationNode, _RegressionNode, _InnerNodeInfo, _NodeInfo,
