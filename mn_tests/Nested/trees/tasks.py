...
@task(fragment=COLLECTION_IN, fragment_buckets=COLLECTION_OUT, range_min=COLLECTION_IN, range_max=COLLECTION_IN)
def filter_fragment(fragment, fragment_buckets, indexes_to_try, num_buckets, range_min=0, range_max=1, indexes_selected=np.array([0])):
    """
    ...
    for index, value in enumerate(indexes_to_try):
        if len(indexes_selected) > 1:
            ...
            actual_fragment = fragment[:, value]
        split_indexes = np.linspace(range_min[0, value], range_max[0, value] + 1, num_buckets + 1)
        ranges = []
        ...
        for _range in ranges:
            if actual_fragment is not None:
                fragment_buckets[index][i] = [k_s_v for k_s_v in actual_fragment if _range[0] <= k_s_v < _range[1]]
            else:
