    ...
    def fit(self, x, y):
        ...
        for i in range(self.distr_depth):
            ...
            for idx, branch_data in enumerate(branches):
                ...
                node_info, results_l, results_l_2, results_r, results_r_2 = _compute_split(x, y, n_classes=self.n_classes, 
        ...
        for branch in branches:
            ...
            actual_node = construct_subtree(x, y, actual_node, self.try_features, self.distr_depth, max_depth=self.max_depth,
                              random_state=self.random_state)
...
class DecisionTreeRegressor(BaseDecisionTree):
    ...
    def fit(self, x, y):
        ...
        for i in range(self.distr_depth):
            ...
            for idx, branch_data in enumerate(branches):
                x, y, actual_node = branch_data
                node_info, results_l, results_l_2, results_r, results_r_2 = _compute_split_regressor(x, y, range_min=self.range_min, range_max=self.range_max,
...
def _compute_split_regressor(x, y, num_buckets=4, range_min=0, range_max=1, indexes_selected=None,
    ...
    for _ in range(number_attributes):
        untried_indices = np.setdiff1d(np.arange(number_attributes), tried_indices)
        index_selection = _feature_selection(
            untried_indices, m_try, random_state
        )
        results = terasort(x, index_selection, range_min=range_min, range_max=range_max, indexes_selected=indexes_selected,
                           num_buckets=num_buckets)
        split_points_per_attribute = []
        for i in range(len(results[0])):
            split_points_per_attribute.append(get_split_point_various_attributes_bucket(results[:, i], number_split_points=number_split_points, split_computation=split_computation))
        ...
        for idx, split_values in enumerate(split_points_per_attribute):
            ...
            if isinstance(x, Array):
                for index_blocks, block_s in enumerate(zip(x._blocks, y._blocks)):
                    ...
                    left_class, right_class = classes_per_split(block_x, block_y, split_values, index_selection,
                                                                  idx_selected[
                ...
                for block_x, block_y in zip(x, y):
                    left_class, right_class = classes_per_split(block_x, block_y, split_values, index_selection, np.array([0]), regression=True)
                    partial_results_left[idx].append(left_class)
                    partial_results_right[idx].append(right_class)
        partial_results_right_array = np.array(compss_wait_on(partial_results_right))
        partial_results_left_array = np.array(compss_wait_on(partial_results_left))
        store_mse_values = []
        ...
        [compss_delete_object(result) for results in partial_results_right for result in results]
        [compss_delete_object(result) for results in partial_results_left for result in results]
        ...
        if (np.sum(left_lengths) + np.sum(right_lengths)) <= 4:
            node_info.set(_compute_leaf_info((np.sum(left_sums) + np.sum(right_sums)) / (np.sum(left_lengths) + np.sum(right_lengths)), None,
                                          occurrences=np.sum(left_lengths) + np.sum(right_lengths)))
        elif np.sum(right_lengths) == 0:
            node_info.set(_compute_leaf_info(
                (np.sum(left_sums) + np.sum(right_sums)) / (np.sum(left_lengths) + np.sum(right_lengths)), None,
                occurrences=np.sum(left_lengths) + np.sum(right_lengths)))
            ...
            node_info.set(_InnerNodeInfo(index_selection[best_attribute], optimal_split_point))
            del right_sums
            del right_lengths
            del left_lengths
            del left_sums
...
def _compute_split(x, y, n_classes=None, num_buckets=4, range_min=0, range_max=1,
    ...
    for _ in range(number_attributes):
        ...
        for i in range(len(
                results[0])):
            split_points_per_attribute.append(get_split_point_various_attributes_bucket(results[:, i],
        ...
        for idx, split_values in enumerate(split_points_per_attribute):
            ...
            if isinstance(x, Array):
                ...
                for block_x, block_y in zip(x, y):
                    left_class, right_class = classes_per_split(block_x, block_y, split_values, index_selection, np.array([0]))
                    partial_results_left[idx].append(left_class)
                    partial_results_right[idx].append(right_class)
        partial_results_right_array = np.array(compss_wait_on(partial_results_right))
        partial_results_left_array = np.array(compss_wait_on(partial_results_left))
        ...
        if isinstance(x, Array):
            for block_x, block_y in zip(x._blocks, y._blocks):
                right_x, right_y, left_x, left_y, aggregate_r, aggregate = apply_split_points_to_blocks(block_x, block_y, best_attribute,
                                                                                optimal_split_point, index_selection, n_classes, aggregate, aggregate_r)
                rights_x.append([right_x])
                rights_y.append([right_y])
                lefts_x.append([left_x])
                lefts_y.append([left_y])
            ...
            for block_x, block_y in zip(x, y):
                right_x, right_y, left_x, left_y, aggregate_r, aggregate = apply_split_points_to_blocks(block_x, block_y,
                                                                                best_attribute,
                                                                                optimal_split_point, index_selection, n_classes, aggregate, aggregate_r)
                rights_x.append([right_x])
                rights_y.append([right_y])
        ...
        if (np.sum(aggregate) + np.sum(aggregate)) <= 4:
            node_info.set(_compute_leaf_info(aggregate + aggregate_r, n_classes))
            del best_attribute
...
def _predict_proba_tree(x, node, node_content_num, n_classes=None, rights=0, depth=0):
    ...
    if isinstance(node_content, _NodeInfo):
        if isinstance(node_content.get(), _LeafInfo):
            ...
            pred = np.empty((x.shape[0], n_classes), dtype=np.float64)
            l_msk = x[:, node_content.get().index:(node_content.get().index + 1)] <= node_content.get().value
            pred[l_msk.flatten(), :] = compss_wait_on(_predict_proba_tree_not_collection(x[l_msk.flatten(), :], node, node_content_num,
...
def _predict_proba_tree_not_collection(x, node, node_content_num, n_classes=None, rights=0, depth=0):
    if node_content_num == 0:
        node_content_num = node_content_num + 1
    else:
        node_content_num = node_content_num * 2 + rights
...
def apply_split_points_to_blocks(x_block, y_block, best_attribute, optimal_value, indexes_to_try, n_classes, aggregate_r, aggregate):
    if optimal_value is None:
        y_block = np.block(y_block)
...
def merge_partial_results_compute_gini_both_sides(partial_results_l, partial_results_r, n_classes):
    ...
    for k in range(len(partial_results_l[0])):
        ...
        for j in range(len(partial_results_l)):
            if len(partial_results_l[j][k]) > 0:
                value_to_concat[:len(partial_results_l[j][k])] = value_to_concat[:len(partial_results_l[j][k])] + \
    ...
    for k in range(len(partial_results_r[0])):
        value_to_concat = np.zeros(n_classes)
        for j in range(len(partial_results_r)):
            value_to_concat[:len(partial_results_r[j][k])] = value_to_concat[:len(partial_results_r[j][k])] + \
    ...
    del concatted_values_r
    return np.array(
        [(number_occurrences_r[i] / (number_occurrences_r[i] + number_occurrences[i]) * gini_values_r[i]) + (
...
def classes_per_split(x_block, y_block, split_points, indexes_to_compare, indexes_to_select=np.array([0]), regression=False):
            ...
            for value in attribute_split_points:
                attribute_splittings_l.append(np.array(
                    [np.mean(y_block[x_block[:, idx] < value, 0]),
                     np.sum(y_block[x_block[:, idx] < value, 0]), len(y_block[x_block[:, idx] < value, 0])]))
...
@constraint(computing_units="${ComputingUnits}")
@task(returns=1)
def get_split_point_various_attributes_bucket(unique_values, number_split_points=100,
                                            split_computation="raw"):
    sample_blocks_list = []
